...
# Pad Function
# -------------------------
pad_vec = lambda x, target: np.pad(x, (0, target - len(x)), 'constant', constant_values=(0.0, 1/topic_dim))
...
# -------------------------
# Feature Extraction (dynamic column support)
# -------------------------
def extract(df):
    ...
    try:
        ...
        topic_vec = pad_vec(topic_features.loc[0, 'table_topic'], topic_dim)
    except Exception as e:
        print(f"[ERROR] Topic feature extraction failed: {e}")
        topic_vec = np.zeros(topic_dim)

    feature_dic['topic'] = torch.FloatTensor(np.tile(topic_vec, (n, 1)))
    ...
    mask = torch.tensor([1] * n, dtype=torch.uint8).view(1, n)
    return feature_dic, mask, n

# -------------------------
# Prediction Pipeline
# -------------------------
def predict_table(df):
    feature_dic, mask, n = extract(df)
    emissions = classifier(feature_dic).view(1, n, -1)
    predictions = model.decode(emissions, mask=mask)
    predicted_types = label_enc.inverse_transform(predictions[0][:n])
...
def main():
    ...
    if args.csv_file:
        ...
        # Setup output directory based on dataset folder name
        dataset_name = os.path.basename(os.path.normpath(args.csv_folder))
    ...
    if args.output_json:
        ...
        if args.csv_folder:
            ...
            output_dir = os.path.join("outputs", dataset_name)
            os.makedirs(output_dir, exist_ok=True)
            json_output_path = os.path.join(output_dir, os.path.basename(args.output_json))
        else:
            json_output_path = args.output_json
        with open(json_output_path, "w") as f:
            json.dump(export_data, f, indent=2)
        print(f"\nSaved predictions to: {json_output_path}")
        export_data = []
        for table_path, col_names, predictions in all_results:
            export_data.append({
                "file": table_path,
